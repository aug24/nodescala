...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      //construct a list of values the same size as the list passed in
      val list = List[T]()
      //construct a future to return
      val p = Promise[List[T]]
      def complete(p: Promise[List[T]], fs: List[Future[T]], ff: List[T]): Unit = fs match {
        case Nil => p.completeWith(Future.always(ff))
        case head :: tail => {
          head onComplete (f => complete(p, tail, ff :+ f.get))
        }
      }
      //set up an onComplete for each future which will poke its completion value into the list
      //and then check if all the values are present.  If they are, complete the promised future
      complete(p, fs, list)
      //return the future
      p.future
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      // construct a promise
      val p = Promise[T]
      // set up an oncomplete from each future to fulfill the promise with the current future
      fs map (f => f onComplete (_ => p.tryCompleteWith(f)))
     ...
     */
    def delay(t: Duration): Future[Unit] = Future {Thread.sleep(t.toMillis); Unit}
